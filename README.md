# CritterCoder

<img src="resources/bigdog.png" style="width:80px; aspect-ratio: fixed;" alt="Small orange and white dog.">

> [!WARNING]
> In development, not done!

CritterCoder is an open source AI coding agent.

This application is an open source coding agent harness inspired by
[ClaudeCode](https://github.com/anthropics/claude-code),
[OpenCode](https://github.com/anomalyco/opencode),
[RolandCode](https://github.com/standardnguyen/rolandcode), and
[OpenClaudia](https://github.com/dollspace-gay/OpenClaudia).

Features:

* 100% open source
* Compatible with multiple model providers
    * State-of-the-art GLM open source models with [Z.ai](https://z.ai/)
    * Local LLM server with [ollama](https://github.com/ollama/ollama)
    * Proprietary models from Anthropic, OpenAI, Google Gemini etc.
* Quick select agent roles and permissions
    * Plan, build, work
    * Write specifications, verify specifications
    * Review code, track down bugs
* Parallel agent tracking and supervision
    * Manage a virtual AI agent team
    * Green team/red team phased work to find problems
    * Keep working towards acceptance criteria
    * Human approval gates
* Persona support
* Agentic tools support
* Stateful memory
    * Automatically track sessions
    * Learn your styles and preferences
* Task tracking
    * Tooling support for issue progress
* Automatic context compaction and optimization
